...
> [!CAUTION]
> 本工具可供 AI Agent 调用以自动化操作上海财经大学的相关业务系统，**LLM 存在幻觉、执行不可控等固有风险**；授权登录后，AI Agent 将以您的**真实用户身份**执行操作（例如发起预约、提交作业等）。我们强烈建议您**仔细核对** Agent 待执行的操作**后再允许其执行**。
>
> **请勿**将包含本地登录状态的 `~/.sufe-cli/state.json` 或包含账号密码的 `~/.sufe-cli/auth.json` 文件**泄露给他人**。
>
